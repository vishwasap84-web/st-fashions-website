...
export async function registerRoutes(
  ...
  app.post("/api/customers/login", async (req, res) => {
    try {
      ...
      const customer = await storage.verifyCustomerPassword(validated.phone, validated.password);
      if (!customer) {
  ...
  app.post("/api/admin/login", async (req, res) => {
    try {
      ...
      const admin = await storage.verifyAdminPassword(validated.username, validated.password);
      if (!admin) {
  ...
  app.post("/api/admin/change-password", async (req, res) => {
    try {
      ...
      const admin = await storage.verifyAdminPassword(validated.username, validated.currentPassword);
      if (!admin) {
        return res.status(401).json({ message: "Current password is incorrect" });
      }
      
      const updated = await storage.updateAdminPassword(validated.username, validated.newPassword);
      if (!updated) {
